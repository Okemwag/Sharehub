...
class Post(DjangoCassandraModel):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.img:
            resize_images.delay(self.img, self.id)
...
class LikePost(DjangoCassandraModel):
    user = columns.UUID(primary_key=True)
    post = columns.UUID(primary_key=True)
    value = columns.Text()

    class Meta:
        get_pk_field = ("user", "post")
